describe('Blog app', function() {
  beforeEach(function() {
    ...
    cy.request('POST', `${Cypress.env('BACKEND')}/api/users`, {
      username: 'Sepi', password: 'salainen'
    }).then(response => {
  ...
  describe('When logged in', function() {
    ...
    it('blogs are ordered by likes', function() {
      cy.createBlog({ title: 'First Blog', author: 'Author A', url: 'https://example.com' })
      cy.createBlog({ title: 'Second Blog', author: 'Author B', url: 'https://example.com' })
      cy.createBlog({ title: 'Third Blog', author: 'Author C', url: 'https://example.com' })
    
      cy.contains('First Blog').parent().find('.visible-btn').as('firstBlogShowInfoButton')
      cy.contains('Second Blog').parent().find('.visible-btn').as('secondBlogShowInfoButton')
      cy.contains('Third Blog').parent().find('.visible-btn').as('thirdBlogShowInfoButton')
    
      cy.get('@firstBlogShowInfoButton').click()
      cy.get('@secondBlogShowInfoButton').click()
      cy.get('@thirdBlogShowInfoButton').click()

      cy.contains('First Blog').parent().find('.like-btn').as('firstBlogLikeButton')
      cy.contains('Second Blog').parent().find('.like-btn').as('secondBlogLikeButton')
      cy.contains('Third Blog').parent().find('.like-btn').as('thirdBlogLikeButton')
